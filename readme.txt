...
PSEUDOCODE
  Loop program untill a "break" occurs
  Get cube side length as a string
  Break if the user inputs "end"
  Try to convert user input to floating point number
  Restart loop if any error (like a Value Error) occurs after sending a short error message
  Calculate the surface area of the cube, and initialize new value as "surfaceArea"
